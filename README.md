...
API: `services/knowledge` on port **8015** — `POST /knowledge/query` (JWT; metered LLM) and LangGraph agent `POST /agent/query` → `{"run_id","answer"}` (optional `session_id` for the guardrail ledger via configurable only, never checkpointed; omitted → ephemeral per-call UUID, not JWT user) plus `GET /agent/trace/{run_id}` and `GET /agent/guardrails/summary` (JWT; omit `session_id` for process-wide counters). Graph: `data/pipelines/support_agent.py` — heuristic `route_sources` (ticket/RAG/both from the question alone; LLM router later), RAG nodes, ticket lookup via **company-tools MCP** (`data/pipelines/tools/ticket_lookup.py` → `check_ticket_status` over Streamable HTTP; Bearer forwarded from `/agent/query` via LangGraph `configurable.access_token`, never checkpointed), plus layered guardrails (`input_guardrails` / `output_guardrails` in `data/pipelines/guardrails.py`: structural, content/scope, security/anti-injection; session ledger for piece-by-piece §3 defense). MCP server: [`mcps/company-tools`](mcps/company-tools/README.md) on port **8016** (incidents create/status/check + read-only inventory; write inventory tools return `INVENTORY_WRITE_FORBIDDEN`). Trace `nodes[]` = attempt order (guardrail blocks include `failure_type`); `final.sources_ran` = contributing sources only (`ticket_lookup` preserved). Evals: `uv run --directory data --python 3.13 pytest` (includes `test_support_agent_evals.py`, `test_agent_guardrails.py` §4 jailbreak cases, + sentinel redaction). Live recon: see [`services/knowledge/README.md`](services/knowledge/README.md). Backoffice UI: `/knowledge`. Env: repo-root `.env.example` (`MCP_SERVER_URL`, `MCP_TICKETS_WRITE_ALLOWLIST`).
