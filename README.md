...
GitHub Actions runs pytest across twelve service/package/MCP directories, the `data/` pipelines suite, a Ruff baseline ratchet that rejects new Python lint debt, Vitest across three npm workspaces, ESLint across five TypeScript workspaces, TypeScript typecheck across four workspaces, and a Prettier format check (see `.github/workflows/ci.yml`).
...
API: `services/knowledge` on port **8015** — `POST /knowledge/query` (JWT; metered LLM) and LangGraph agent `POST /agent/query` → `{"run_id","answer","memory_proposal"?}` (optional `session_id` for the guardrail ledger + pending memory via configurable only, never checkpointed; omitted → ephemeral per-call UUID, not JWT user) plus `GET /agent/trace/{run_id}`, `GET /agent/guardrails/summary`, `GET /agent/memory`, and `GET /agent/memory/audit` (JWT). Graph: `data/pipelines/support_agent.py` — heuristic `route_sources` (ticket/RAG/both), RAG + ticket MCP, guardrails, and confirmation-gated agent memory (`resolve_memory` / `attach_memory_proposal`; Redis-backed location+category store via `data/pipelines/memory_store.py`; `REDIS_URL`). MCP server: [`mcps/company-tools`](mcps/company-tools/README.md) on port **8016**. Evals: `uv run --directory data --python 3.13 pytest` (includes `test_support_agent_evals.py`, `test_agent_guardrails.py`, `test_agent_memory.py`). Live recon: see [`services/knowledge/README.md`](services/knowledge/README.md). Backoffice UI: `/knowledge`. Env: repo-root `.env.example` (`MCP_SERVER_URL`, `MCP_TICKETS_WRITE_ALLOWLIST`, `REDIS_URL`).
