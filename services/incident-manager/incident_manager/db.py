"""TinyDB access for the incident manager.

The database is a lazy module-level singleton: the first call to ``get_db()``
opens ``data/incidents.json`` (or an explicit path) and later calls reuse the
...
``INCIDENT_DB_PATH`` overrides the JSON file location and is intended for tests
and local tooling only. Leave it unset in production shells so the default
``data/incidents.json`` under this service is used.
"""
...
_DEFAULT_PATH = Path(__file__).resolve().parent.parent / "data" / "incidents.json"
...
def _resolve_path(path: Path | None) -> Path:
    if path is not None:
        return path
    env_path = os.environ.get("INCIDENT_DB_PATH")
    if env_path:
